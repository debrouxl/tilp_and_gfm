...
int ti85_gtl_to_tigl_other_files(char *filename_src, char *filename_dst);
int ti85_gtl_to_tigl_group_files(char *filename_src, char *filename_dst);
int ti85_gtl_to_tigl_backup_files(char *filename_src, char *filename_dst);

/* ------------------------------------ */
/* tilp files to TIGraphLink files */
/* ------------------------------------ */

int ti85_gtl_to_tigl(char *filename_src, char *filename_dst)
{
  FILE *in;
  char varname[9];
  char svartype[5];
  ...
  fscanf(in, "%s\n", varname);
  if(!strcmp(varname, "TI85_PAK"))
  {
	/* We have a PAK file -> .85g */
	ti85_gtl_to_tigl_group_files(filename_src, filename_dst);
	fclose(in);
  }
  else
  {
	rewind(in);
	fscanf(in, "%s\n", varname);
	if(strcmp(varname, "TI85")) return 26;
	fscanf(in, "%s\n", varname);
	while(fgetc(in) != '\n');
	fscanf(in, "%s\n", svartype);
	if(!strcmp(svartype, "BACKUP"))
	{
	  ...
	  ti85_gtl_to_tigl_backup_files(filename_src, filename_dst);
	}
	else
	{
	  /* We have a normal file -> .85? */
	  ti85_gtl_to_tigl_other_files(filename_src, filename_dst);
	}
	fclose(in);
  }
...
int ti85_gtl_to_tigl_group_files(char *filename_src, char *filename_dst)
{
  ...
  rewind(in);
  fscanf(in, "%s\n", buffer);
  if(strcmp(buffer, "TI85_PAK")) return 26;
  fscanf(in, "%08X\n", &nvar);
...
int ti85_tigl_to_gtl(char *filename_src, char *filename_dst)
{
  FILE *in;
  FILE *out=NULL;
  char varname[9];
  ...
  if(in == NULL) return 37;
  fgets(buffer, 9, in);
  if(strcmp(buffer, "**TI85**")) return 26;
  fgetc(in);
  fgetc(in);
  fgetc(in);
