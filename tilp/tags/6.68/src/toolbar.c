...
//gdk_pixbuf_new_from_xpm_data    (const char **data);
	...
	old_icon = child->icon;
	new_icon =
	    create_pixmap(toolbar_win.toolbar, "tb_calc_screen.xpm");
	pixbuf = gtk_image_get_pixbuf(GTK_IMAGE(new_icon));
	gtk_image_set_from_pixbuf(GTK_IMAGE(old_icon), pixbuf);
	child = g_list_nth(tb->children, 2)->data;
	old_icon = child->icon;
	new_icon =
	    create_pixmap(toolbar_win.toolbar, "tb_calc_dirlist.xpm");
	pixbuf = gtk_image_get_pixbuf(GTK_IMAGE(new_icon));
	gtk_image_set_from_pixbuf(GTK_IMAGE(old_icon), pixbuf);
	child = g_list_nth(tb->children, 3)->data;
	old_icon = child->icon;
	new_icon =
	    create_pixmap(toolbar_win.toolbar, "tb_calc_backup.xpm");
	pixbuf = gtk_image_get_pixbuf(GTK_IMAGE(new_icon));
	gtk_image_set_from_pixbuf(GTK_IMAGE(old_icon), pixbuf);
	child = g_list_nth(tb->children, 4)->data;
	old_icon = child->icon;
	new_icon =
	    create_pixmap(toolbar_win.toolbar, "tb_calc_restore.xpm");
	pixbuf = gtk_image_get_pixbuf(GTK_IMAGE(new_icon));
	gtk_image_set_from_pixbuf(GTK_IMAGE(old_icon), pixbuf);
...
void toolbar_refresh_buttons(void)
{
	...
	gtk_widget_set_sensitive(toolbar_win.button10,
				 (ti_calc.
				  supported_operations() & OPS_ISREADY));
	gtk_widget_set_sensitive(toolbar_win.button11,
				 (ti_calc.
				  supported_operations() & OPS_DIRLIST));
	gtk_widget_set_sensitive(toolbar_win.button12,
				 (ti_calc.
				  supported_operations() &
				  OPS_RECV_BACKUP));
	gtk_widget_set_sensitive(toolbar_win.button13,
				 (ti_calc.
				  supported_operations() &
				  OPS_SEND_BACKUP));
