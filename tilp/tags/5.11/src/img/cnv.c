...
/* Pass from 1 bit/pixel to 1 byte/pixel and fill a colormap */
...
{
  //(img->bitmap) = (byte *)malloc(((img->width) * (img->height)) >> 3);
	(img->bitmap) = (byte *)calloc(((img->width) * (img->height)) >> 3, sizeof(byte));
  if(img->bitmap == NULL)
    exit(-1);
}
...
void alloc_bytemap(Image *img)
{
  //(img->bytemap) = (byte *)malloc((img->width) * (img->height));
	(img->bytemap) = (byte *)calloc((img->width) * (img->height), sizeof(byte));
  if(img->bytemap == NULL)
    exit(-1);
}
...
void alloc_colormap(Image *img)
{
  //img->colormap = (byte *)malloc(3*256 * sizeof(byte));
	img->colormap = (byte *)calloc(3*256, sizeof(byte));
  if(img->colormap == NULL)
    exit(-1);
}
...
void alloc_rgbmap(Image *img)
{
  //(img->rgbmap) = (byte *)malloc(3 * (img->width) * (img->height));
	(img->rgbmap) = (byte *)calloc(3 * (img->width) * (img->height), sizeof(byte));
  if(img->rgbmap == NULL)
    exit(-1);
}
