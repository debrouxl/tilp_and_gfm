...
int ti82_gtl_to_tigl_other_files(char *filename_src, char *filename_dst);
int ti82_gtl_to_tigl_group_files(char *filename_src, char *filename_dst);
int ti82_gtl_to_tigl_backup_files(char *filename_src, char *filename_dst);

/* ------------------------------------ */
/* tilp files to TIGraphLink files */
/* ------------------------------------ */

int ti82_gtl_to_tigl(char *filename_src, char *filename_dst)
{
  FILE *in;
  char varname[9];
  char svartype[5];
  ...
  fscanf(in, "%s\n", varname);
  if(!strcmp(varname, "TI82_PAK"))
  {
	/* We have a PAK file -> .82g */
	ti82_gtl_to_tigl_group_files(filename_src, filename_dst);
	fclose(in);
  }
  else
  {
	rewind(in);
	fscanf(in, "%s\n", varname);
	if(strcmp(varname, "TI82")) return 26;
	fscanf(in, "%s\n", varname);
	while(fgetc(in) != '\n');
	fscanf(in, "%s\n", svartype);
	if(!strcmp(svartype, "BACKUP"))
	{
	  ...
	  ti82_gtl_to_tigl_backup_files(filename_src, filename_dst);
	}
	else
	{
	  /* We have a normal file -> .82? */
	  ti82_gtl_to_tigl_other_files(filename_src, filename_dst);
	}
	fclose(in);
  }
...
int ti82_gtl_to_tigl_group_files(char *filename_src, char *filename_dst)
{
  ...
  rewind(in);
  fscanf(in, "%s\n", buffer);
  if(strcmp(buffer, "TI82_PAK")) return 26;
  fscanf(in, "%08X\n", &nvar);
...
int ti82_tigl_to_gtl(char *filename_src, char *filename_dst)
{
  FILE *in;
  FILE *out=NULL;
  char varname[9];
  ...
  if(in == NULL) return 37;
  fgets(buffer, 9, in);
  if(strcmp(buffer, "**TI82**")) return 26;
  fgetc(in);
  fgetc(in);
  fgetc(in);
