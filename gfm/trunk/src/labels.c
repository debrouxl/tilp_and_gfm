...
void labels_set_ram(uint32_t ram)
{
	...
	if (ram < 1024)
		str = g_strdup_printf(_("%u bytes"), ram);
...
void labels_set_flash(uint32_t flash)
{
	...
	if (flash < 1024)
		str = g_strdup_printf(_("%u bytes"), flash);
...
void labels_refresh(void)
{
	...
	n = ticalcs_dirlist_ve_count(GFMFile.trees.vars) + ticalcs_dirlist_ve_count(GFMFile.trees.apps);
	ram = (unsigned int)ticalcs_dirlist_ram_used(GFMFile.trees.vars);
	flash = (unsigned int)ticalcs_dirlist_flash_used(GFMFile.trees.vars, GFMFile.trees.apps);
